...
//        outputLoadingScreen.startThread();
	...
	public void outputMainLayout() {
		...
		mainLayout.addRule();
		mainLayout.addRow("[*]\tDisplay", "[W|w]\tWrite", "[R|r]\tRead", "[U|u]\tUpdate", "[D|d]\tDelete", "[F|f]\tFirst", "[P|p]\tPrevious", "[N|n]\tNext","[S|s]\tSearch");
		mainLayout.addRule();
		mainLayout.addRow("[L|l]\tLast", "[G|g]\tGoto", "[O|o]\tSet Row", "[V|v]\tSave", "[C|c]\tBack up", "[T|t]\tRestore", "[H|h]\tHelp", "[E|e]\tExit",".....");
		mainLayout.addRule();

		mainLayout.getContext().setWidth(150);
...
GetConnection getConnection = new GetConnection();
		char choice;
		int id = getConnection.getLastRecord();
	...
	public void searchDataLayout() {
		...
		String productName = TextFieldConsole.readStringType("Product Name       : ");
		ArrayList resultList = Recover.findRecordByName(productName, listOfProducts);
...
//        file.delete();
	...
	public boolean backupDataToFileProcess() {
		...
		try {
			...
			String script = "INSERT INTO tbproduct (proID,proName,proUnitPrice,proQty,importDate,status)";
			String script2 = "VALUES(";
			Statement statement = GetConnection.getConnection().createStatement();
			ResultSet resultSet = statement.executeQuery(sql);

			while (resultSet.next()) {
				String value = resultSet.getInt(1) + "," + "'" + resultSet.getString(2) + "'" + "," + resultSet.getDouble(3) + "," + resultSet.getInt(4) + "," + "'" + resultSet.getString(5) + "'"+","+resultSet.getInt(6);
	...
	public boolean restoreDataToFileProcess() {
		...
		try {
			...
			if (indexOfFile < 0 || indexOfFile > result.size()) {
				return false;
			}
